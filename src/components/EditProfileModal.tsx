...
import { User, Mail, Briefcase, Phone, MapPin, Calendar, Lock, Loader2, ShieldCheck, KeyRound, X } from "lucide-react";
...
const profileUpdateSchema = z.object({
  username: z.string().min(2, "Identity alias must contain at least 2 characters"),
  ...
  address: z.string().min(4, "Operational nexus physical tracking footprint too brief"),
...
const passwordUpdateSchema = z
  .object({
    ...
    newPassword: z.string().min(6, "New access sequence must be at least 6 characters"),
    confirmNewPassword: z.string().min(6, "Sequence conformation parity check required"),
  })
...
export default function EditProfileModal({ isOpen, onClose, profile }: EditProfileModalProps) {
  ...
  const profileForm = useForm<ProfileFormValues>({
    ...
    defaultValues: { username: "", profession: "", phoneNumber: "", address: "", dateOfBirth: "" },
  });
  ...
  const profileMutation = useMutation({
    mutationFn: async (values: ProfileFormValues) => {
      if (!profile?._id) throw new Error("Invalid operational token target instance ID.");
      ...
      if (!response.ok) {
        ...
        throw new Error(err?.message || "Failed to commit corporate registry attributes.");
      }
    ...
    onSuccess: () => {
      ...
      toast.success({
        ...
        description: "Corporate matrix core records updated and locked dynamically.",
      });
      onClose();
    },
    onError: (error: Error) => {
      toast.error({ title: "Profile Modification Rejected", description: error.message });
    },
  });
  ...
  const passwordMutation = useMutation({
    mutationFn: async (values: PasswordFormValues) => {
      ...
      if (!response.ok) {
        const err = await response.json().catch(() => null);
        throw new Error(err?.message || "Cryptographic rotation sequence handshake aborted.");
      }
      return response.json();
    },
    onSuccess: () => {
      toast.success({
        ...
        description: "New authorization secure cipher token applied to core node.",
      });
      passwordForm.reset();
      onClose();
    },
    onError: (error: Error) => {
      toast.error({ title: "Rotation Handshake Failed", description: error.message });
    },
  });
        ...
        <div className="p-6 mt-4">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            {/* ==========================================
                ...
                ========================================== */}
            <TabsContent value="identity" className="space-y-4 focus-visible:outline-none focus-visible:ring-0">
              <DialogHeader className="mb-2">
                <DialogTitle className="text-xl font-bold tracking-tight flex items-center gap-2">
                  <ShieldCheck className="h-5 w-5 text-blue-500" /> Synchronize Enterprise Scope
                </DialogTitle>
              </DialogHeader>

              <Form {...profileForm}>
                <form onSubmit={profileForm.handleSubmit((data) => profileMutation.mutate(data))} className="space-y-4 pt-2">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="font-semibold text-[11px] uppercase tracking-wider text-slate-400">User Handle Alias</FormLabel>
                          <FormControl>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="font-semibold text-[11px] uppercase tracking-wider text-slate-400">Corporate Profile Role</FormLabel>
                          <FormControl>
                            <div className="relative group">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="font-semibold text-[11px] uppercase tracking-wider text-slate-400">Secure Comms Line</FormLabel>
                          <FormControl>
                            <div className="relative group">
                  ...
                  <Button
                    ...
                    className={cn(
                      ...
                      profileMutation.isPending ? "bg-slate-800 text-slate-400 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-500 text-white",
                    )}
                    disabled={profileMutation.isPending}>
                    {profileMutation.isPending ? (
                      <>
                        <Loader2 className="mr-2 h-5 w-5 animate-spin" /> Synchronizing Matrix Ledger...
                      </>
                    ) : (
            ...
            <TabsContent value="security" className="space-y-4 focus-visible:outline-none focus-visible:ring-0">
              <DialogHeader className="mb-2">
                <DialogTitle className="text-xl font-bold tracking-tight flex items-center gap-2">
                  <Lock className="h-5 w-5 text-rose-500" /> Rotate Cipher Credentials
                </DialogTitle>
              </DialogHeader>

              <Form {...passwordForm}>
                <form onSubmit={passwordForm.handleSubmit((data) => passwordMutation.mutate(data))} className="space-y-4 pt-2">
                  <FormField
                    control={passwordForm.control}
                    name="oldPassword"
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="font-semibold text-[11px] uppercase tracking-wider text-slate-400">Current Secret Key Cipher</FormLabel>
                        <FormControl>
                          <div className="relative group">
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="font-semibold text-[11px] uppercase tracking-wider text-slate-400">Generate New Cipher</FormLabel>
                          <FormControl>
                            <div className="relative group">
                              <Lock className="absolute left-3 top-3 h-4 w-4 text-slate-500 group-focus-within:text-rose-500 transition-colors" />
                              <Input
                  ...
                  <Button
                    ...
                    className={cn(
                      "w-full h-12 font-bold tracking-wide text-[15px] mt-6 shadow-md transition-all active:scale-[0.99]",
                      passwordMutation.isPending ? "bg-slate-800 text-slate-400 cursor-not-allowed" : "bg-rose-600 hover:bg-rose-500 text-white",
                    )}
                    disabled={passwordMutation.isPending}>
                    {passwordMutation.isPending ? (
                      <>
                        <Loader2 className="mr-2 h-5 w-5 animate-spin" /> Reparsing Security Node Credentials...
                      </>
                    ) : (
